...
export default function LineChart({
  data = [42, 58, 63, 71, 68, 54, 77, 83, 61],
  threshold = 65,
  className = "h-full",
  lineLabelsRef,
  ...props
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (chartRef.current) {
        const baseTime = Date.now() - (data.length - 1) * 20000;
        const timestamps = Array.from({ length: data.length }, (_, i) => baseTime + i * 20000);
        const dataPoints = data.map((y, i) => ({ x: timestamps[i], y }));
        ...
        chart = new ApexCharts(chartRef.current, lineOptions);
        chart.render();
      }
    };
  ...
  return (
    <div className="relative h-full">
      ...
      {lineLabelsRef && <div ref={lineLabelsRef} className="pointer-events-none absolute inset-0" />}
    </div>
  );
}
